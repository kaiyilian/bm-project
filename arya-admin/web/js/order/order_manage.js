...
var $soin_area_modal = $(".soin_area_modal");//参保地区 弹框
var $soin_pay_fail_modal = $(".soin_pay_fail_modal");//缴纳失败 原因
var $soin_pay_fail_all_modal = $(".soin_pay_fail_all_modal");//
var $order_detail_modal = $(".order_detail_modal");//订单详情
var $order_modify_modal = $(".order_modify_modal");//订单 批量修改弹框
...
var order_manage = {
	...
	batch_list: [],//批量修改的订单 idVersions

	//初始化
	Init: function () {
		order_manage.containerName = ".order_manage_container";

		order_manage.initMonthChoose();//初始化 月份  选择
		order_manage.initSearchCondition();//一键重置
		...
		//订单 批量修改 弹框显示
		$order_modify_modal.on("shown.bs.modal", function () {

			order_manage.batch_list = [];
			...
			for (var i = 0; i < $item_active.length; i++) {
				...
				order_manage.batch_list.push(obj);

			}

			order_manage.orderModifyOnceData();//获取 批量修改的数据的 第一条数据
	...
	initSearchCondition: function () {
		order_manage.initUserType();//初始化 用户类型
		order_manage.initSoinArea();//初始化 参保地区
		order_manage.initSupplierList("");//初始化 供应商列表
		order_manage.initPayStatus();//初始化 缴纳状态
		order_manage.initIncreaseOrDecrease();//初始化 增减员
		//order_manage.IsChooseAll();//是否 已经全部选择

		var $search_container = $(order_manage.containerName).find(".search_container");
		//初始化 月份  选择
		$search_container.find(".order_month").html("请选择开始月份");
		$search_container.find(".order_end_month").html("请选择结束月份");
		order_manage.alreadyChooseMonth = null;
		order_manage.alreadyEndChooseMonth = null;
		...
		var order_list = "<tr><td colspan='14'>暂无订单信息</td></tr>";
	...
	initSoinArea: function () {
		...
		//获取 地区列表
		initAreaTree(
			".soin_area_modal .soin_all_area_tree",//tree id
			".soin_area_modal .all_area_tree_hud",//动画 id
			urlGroup.order_manage_district_list_get_url,
			function (treeNode) {
				order_manage.ChooseCity(treeNode);
			}
		);
	...
	initMonthChoose: function () {
		$(order_manage.containerName).find(".order_month")
			.datepicker({
				minViewMode: 1,
			...
			.on("changeMonth", function (e) {
				...
				//选择了不同的月份
				if (yearMonth != order_manage.alreadyChooseMonth) {
					order_manage.alreadyChooseMonth = yearMonth;
					$(order_manage.containerName).find(".order_month").html(yearMonth);
				}
				else {
					$(order_manage.containerName).find(".order_month").html("请选择开始月份");
					order_manage.alreadyChooseMonth = null;
				}

			});
		$(order_manage.containerName).find(".order_end_month")
			.datepicker({
				minViewMode: 1,
				keyboardNavigation: false,
				forceParse: false,
			...
			.on("changeMonth", function (e) {
				...
				//选择了不同的月份
				if (yearMonth != order_manage.alreadyEndChooseMonth) {
					order_manage.alreadyEndChooseMonth = yearMonth;
					$(order_manage.containerName).find(".order_end_month").html(yearMonth);
				}
				else {
					$(order_manage.containerName).find(".order_end_month").html("请选择结束月份");
					order_manage.alreadyEndChooseMonth = null;
				}
	...
	initIncreaseOrDecrease: function () {
		...
		var option =
			...
			"<option value='4'>在缴</option>" +
			"<option value='0'>正常（顺延）</option>" +
	...
	//参保地区 - 弹框显示
	SoinAreaModalShow: function () {
		$soin_area_modal.modal("show");
	},
	//选中对应的城市
	ChooseCity: function (treeNode) {
		order_manage.currentTreeNode = treeNode;
	},
	//参保地区 - 确认选中
	SoinAreaSure: function () {
		//如果没有选择 地区
		if (order_manage.currentTreeNode == "") {
			toastr.warning("请选择地区！");
			return;
		}

		$soin_area_modal.modal("hide");

		loadingInit();//加载框 出现

		var id = order_manage.currentTreeNode.id;//地区id
		var name = order_manage.currentTreeNode.name;

		$(order_manage.containerName).find(".search_container .soin_area")
			.val(name).attr("data-district_id", id);

		order_manage.initSupplierList(id);//初始化 供应商列表
	},

	//订单查询 - 按钮点击
	btnSearchClick: function () {
		order_manage.currentPage = 1;
		order_manage.orderList();//订单查询 - 列表
	},
	//排序
	orderSort: function (self) {
		...
		order_manage.sort_code = sort_code;
		order_manage.sort_name = sort_name;
		order_manage.btnSearchClick();//查询
	},
	//订单查询 - 列表
	orderList: function () {
		var $table_container = $(order_manage.containerName).find(".table_container");
		...
		//请求
		if (!order_manage.alreadyChooseMonth && !order_manage.alreadyEndChooseMonth) {
			toastr.warning("请选择服务开年月或结束年月!");
			return;
		}
		if (order_manage.alreadyChooseMonth != null && order_manage.alreadyEndChooseMonth != null) {
			var b_time = Number(order_manage.alreadyChooseMonth);
			var e_time = Number(order_manage.alreadyEndChooseMonth);
			if (b_time > e_time) {
		...
		order_manage.searchConditionSet();//赋值 查询条件
		...
		obj.sort_name = order_manage.sort_name;
		obj.sort_code = order_manage.sort_code;
		obj.page = order_manage.currentPage;
		obj.page_size = "20";
			...
			function (data) {
				...
				if (data.code == ERR_CODE_OK) {
					...
					if (data.result) {
						...
						order_manage.totalPage = data.result["pages"];

						var order_list = "";
						var orders = data.result.orders;//
						if (!orders || orders.length == 0) {
							order_list = "<tr>" +
								"<td colspan='14'>暂无订单信息</td>" +
								"</tr>";
						}
						else {
							$.each(orders, function (index, orders) {
								...
								var order_version = orders.version ? orders.version : 0;
								...
								var order_collection_total = orders.total_in;// 收账总计
								var order_charge_total = orders.total_out;// 出账总计
								...
								order_list += "<tr class='item order_item'" +
									...
									"onclick='order_detail.orderDetailModalShow(this)'" +
									">" +
									"<td class='choose_item' onclick='order_manage.chooseItem(this,event)'>" +
									...
									"<td class='order_pay_month'>" + order_pay_month_content + "</td>" +
									"<td class='order_back_pay_month'>" + order_back_month_content + "</td>" +
	...
	orderListInit: function () {
		...
		var $item = $(order_manage.containerName).find("tbody .item");
		var $pager_container = $(order_manage.containerName).find(".pager_container");
		...
		var options = {
			...
			currentPage: order_manage.currentPage, //当前页数
			totalPages: order_manage.totalPage, //总页数
			...
			onPageClicked: function (event, originalEvent, type, page) { //点击事件
				...
				order_manage.currentPage = page;
				order_manage.orderList();//查询 满足条件的订单
	...
	searchConditionSet: function () {
		...
		//选择的月份
		var choose_month = order_manage.alreadyChooseMonth ?
			order_manage.alreadyChooseMonth : "";
		var choose_end_month = order_manage.alreadyEndChooseMonth ?
			order_manage.alreadyEndChooseMonth : "";
		//订单状态id
	...
	//订单批量修改 弹框显示
	orderModifyModalShow: function () {
		var $table_container = $(order_manage.containerName).find(".table_container");
		var $item_active = $table_container.find("tbody .item.active");//tbody item

		if ($item_active.length <= 0) {
			toastr.warning("请先选择订单！");
			return
		}

		$order_modify_modal.modal("show");
	},
	//获取 批量修改的数据的 第一条数据
	orderModifyOnceData: function () {
		if (order_manage.batch_list.length <= 0) {
			return
		}
		...
		var obj = {};
		obj.id = order_manage.batch_list[0].id;
		var url = urlGroup.order_modify_one_data + "?" + jsonParseParam(obj);
			...
			function (data) {
				...
				if (data.code == RESPONSE_OK_CODE) {
					...
					if (data.result) {
						...
						var $item = $order_modify_modal.find("table tbody .item");

						//补充公积金
						var houseFundAdditionTotal = $result.houseFundAdditionTotal ? $result.houseFundAdditionTotal : "0";
						//残保
						var disabilityTotal = $result.disabilityTotal ? $result.disabilityTotal : "0";
						//大病医疗
						var serverIllnessTotal = $result.serverIllnessTotal ? $result.serverIllnessTotal : "0";
						//工伤补充
						var injuryAdditionTotal = $result.injuryAdditionTotal ? $result.injuryAdditionTotal : "0";
						//采暖费
						var heatingTotal = $result.heatingTotal ? $result.heatingTotal : "0";

						$item.find(".disabilityTotal").attr("data-val", disabilityTotal)
							.find("div").html(disabilityTotal);
						$item.find(".serverIllnessTotal").attr("data-val", serverIllnessTotal)
							.find("div").html(serverIllnessTotal);
						$item.find(".injuryAdditionTotal").attr("data-val", injuryAdditionTotal)
							.find("div").html(injuryAdditionTotal);
						$item.find(".heatingTotal").attr("data-val", heatingTotal)
							.find("div").html(heatingTotal);
						$item.find(".houseFundAdditionTotal")
							.attr("data-val", houseFundAdditionTotal)
							.find("div").html(houseFundAdditionTotal);

						//赋值 初始化
						$order_modify_modal.find(".modal-body table .npt_val").each(function () {

							$(this).dblclick(function () {

								var $npt = $("<input>");
								$npt.addClass("form-control");
								$npt.attr("placeholder", $(this).attr("data-val"));
								$npt.attr("onblur", "order_manage.orderModifyRemoveBlur(this)");
								$npt.keyup(function () {
									...
									if (param.length > 1) {
										...
										if (val.length > 2) {
											val = val.substr(0, 2);
											this.value = param[0] + "." + val;
										}
									}

								});
								$npt.appendTo($(this).find("div").empty());
								$npt.focus();
	...
	//赋值输入框 移除焦点
	orderModifyRemoveBlur: function (self) {
		var reg = /^(?!0[0-9])\d+((\.\d{1,2})?)$/;
		var default_val = $(self).closest(".npt_val").attr("data-val");//默认值 （原来的值）
		...
		$(self).closest(".npt_val").attr("data-val", val);
		$(self).closest(".npt_val").find("div").html(val);
	...
	orderSaveByModify: function () {
		...
		var obj = {};
		obj.batch = order_manage.batch_list;
		obj.disabilityTotal = $item.find(".disabilityTotal div").html();
		obj.serverIllnessTotal = $item.find(".serverIllnessTotal div").html();
		obj.injuryAdditionTotal = $item.find(".injuryAdditionTotal div").html();
		obj.heatingTotal = $item.find(".heatingTotal div").html();
		obj.houseFundAdditionTotal = $item.find(".houseFundAdditionTotal div").html();

		aryaPostRequest(
			urlGroup.order_save_by_modify,
			obj,
			function (data) {
				//console.log("获取日志：");
				//console.log(data);
	...
	orderExport: function () {
		...
		if ($(order_manage.containerName).find("tbody .item").length <= 0) {
		...
		exportWarning("全部订单", function () {
			...
			form.attr("action", urlGroup.order_manage_export_url);
...
var order_detail = {
	...
	init: function () {
		...
		//订单详情 弹框显示
		$order_detail_modal.on("shown.bs.modal", function () {
	...
	//获取月份列表
	monthList: function () {
		var $modal_body = $order_detail_modal.find(".modal-body");
					...
					if (!arr || arr.length == 0) {
						...
						for (var i = 0; i < arr.length; i++) {
							...
							list +=
								...
								"onclick='order_detail.orderDetail(this)'>" +
...
$(function () {
	...
	//显示 排序icon
	$(".order_manage_container .content .table_container table thead tr td").each(function () {
		$(this).hover(
			function () {
...
var debug = {
	...
	orderList: function () {
		...
		//请求
		if ((order_manage.alreadyChooseMonth == null ||
			order_manage.alreadyChooseMonth == "") &&
			(order_manage.alreadyEndChooseMonth == null ||
			order_manage.alreadyEndChooseMonth == "")) {
			toastr.warning("请选择服务开年月或结束年月!");
		...
		$item.each(function () {
			...
			if (status == 1) {		//成功
				...
				pay_status_spn = "<span class='txt'>已缴纳成功</span>" +
					"<span class='btn btn-sm btn-primary' " +
					"onclick='order_manage.payFailOnly(this)'>缴纳失败</span>"
			}
			else if (status == 2) {		//失败
				$(this).addClass("pay_fail");

				pay_status_spn = "<span class='txt' data-fail_reason='" + reason + "' " +
					"onclick='order_manage.payFailMsgShow(this)'>已缴纳失败</span>" +
					"<span class='btn btn-sm btn-primary' " +
					"onclick='order_manage.paySuccessOnly(this)'>缴纳成功</span>"
			}
			else if (status == 3) {  //暂未缴纳 (显示 缴纳失败、缴纳成功)
				...
				var input = $("<input>").val(val).blur(function () {
					...
					aryaPostRequest(
						...
						function (data) {
							...
							if (data.code == 1000) {
								toastr.success("编辑成功！");
								order_manage.orderList();//重新获取当前页数据
							}
							else {
								messageCue(data.msg);
							}
	...
	IsChooseAll: function () {
		...
		//没有全部选中
		if (chooseNo == 0 ||
			chooseNo < $(order_manage.containerName)
				.find("tbody .item").length) {
			$(order_manage.containerName).find(".choose_container").removeClass("active")
				.find("img").attr("src", "img/icon_Unchecked.png");
		}
		else {
			$(order_manage.containerName).find(".choose_container").addClass("active")
				.find("img").attr("src", "img/icon_checked.png");
		}
	...
	btnListInit: function () {
		var $foot = $(order_manage.containerName).find(".foot");
		//选中的item
		var $activity_item = $(order_manage.containerName).find("tbody .item.active");
		//选中的 缴纳失败的订单
		var $pay_fail_item = $(order_manage.containerName).find("tbody .item.active.pay_fail");
		//选中的 缴纳成功的订单
		var $pay_success_item = $(order_manage.containerName).find("tbody .item.active.pay_success");
		//当前查询条件 - 缴纳状态
		var pay_status = $(order_manage.containerName).find(".search_container")
	...
	payFailOnly: function (self) {
		...
		order_manage.PayFailArray = "{'id':'" + id + "'}";
		order_manage.PayFailArray = "[" + order_manage.PayFailArray + "]";
		order_manage.PayFailArray = eval("(" + order_manage.PayFailArray + ")");

		order_manage.payFailModalShow();//缴纳失败 - 弹框显示

	},
	//缴纳失败 - 多个
	payFailMore: function () {
		var $item = $(order_manage.containerName).find("tbody .item.active");
		if ($item.length <= 0) {
			toastr.info("请选择订单！");
			return;
		}
		...
		$item.each(function () {
			...
			order_manage.PayFailArray += order_manage.PayFailArray == "" ? ("{'id':'" + id + "'}")
				: (",{'id':'" + id + "'}")
		});
	...
	payFailSure: function () {
		...
		var obj = new Object();
		obj.ids = order_manage.PayFailArray;
		obj.reason = reason;
		...
		aryaPostRequest(
			...
			function (data) {
				...
				if (data.code == RESPONSE_OK_CODE) {
					toastr.success("缴纳失败操作成功！");
					$soin_pay_fail_modal.modal("hide");
					order_manage.PayFailArray = "";//初始化 赋值
					order_manage.btnSearchClick();//订单列表获取
				}
				else {
					messageCue(data.msg);
				}
	...
	paySuccessOnly: function (self) {
		...
		order_manage.PaySuccessArray = "{'id':'" + id + "'}";
		order_manage.PaySuccessArray = "[" + order_manage.PaySuccessArray + "]";
		order_manage.PaySuccessArray = eval("(" + order_manage.PaySuccessArray + ")");

		order_manage.paySuccessSure();//缴纳成功 - 确认 弹框显示
	...
	paySuccessSure: function () {
		...
		swal(
			...
			function () {
				...
				var obj = new Object();
				obj.ids = order_manage.PaySuccessArray;
				console.log(obj);
				...
				aryaPostRequest(
					...
					function (data) {
						...
						if (data.code == RESPONSE_OK_CODE) {
							toastr.success("缴纳成功 操作成功！");
							order_manage.PaySuccessArray = "";//初始化 赋值
							order_manage.btnSearchClick();//订单列表获取
	...
	orderDelByOnly: function (self) {
		...
		order_manage.DelArray = "{'id':'" + id + "'}";
		order_manage.DelArray = "[" + order_manage.DelArray + "]";
		order_manage.DelArray = eval("(" + order_manage.DelArray + ")");

		order_manage.orderDel();//订单删除 - 确认
	},
	//订单删除 - 多项删除
	orderDelByMore: function () {
		...
		$item.each(function () {
			...
			order_manage.DelArray += order_manage.DelArray == "" ? ("{'id':'" + id + "'}")
				: (",{'id':'" + id + "'}")
		});
